 ...
 * Only AES-128/AES-192/AES-256 key generation is supported
 * output_key: constraint condition:
 * output_key->size
 * key_param: The parameter of the key which need to generate
 * constraint condition:
 * key_param cannot be NULL
 * key_param->key_type must be HKS_KEY_TYPE_AES
 * return 0 OK, other error
 */
...
/*
 * encrypt
 * only support AES-128/AES-192/AES-256 GCM/CCM/CBC encrypt
 * key: used to ecrypt plain_text
 * key_param: constraint condition:
 * key_param.key_type is HKS_KEY_TYPE_AES;
 * key_param.key_len is 128 or 192 or 256;
 * key_param.key_usage is HKS_KEY_USAGE_ENCRYPT;
 * key_param.key_mode is HKS_ALG_GCM or HKS_ALG_CBC or HKS_ALG_CCM;
 * key_param.key_pad is HKS_PADDING_NONE or HKS_PADDING_PKCS7;
 * crypt_param:
 * crypt_param.nonce.size need set different values due to key_mode
 * crypt_param.aad.size need set different values due to key_mode
 * return 0 OK, other error
 */
...
/*
 * decrypt
 * only support AES-128/AES-192/AES-256 GCM/CCM/CBC decrypt
 * key: used to decrypt cipher_text_with_tag
 * key_param: constraint condition:
 * key_param.key_type is HKS_KEY_TYPE_AES;
 * key_param.key_len is 128 or 192 or 256;
 * key_param.key_usage is HKS_KEY_USAGE_DECRYPT;
 * key_param.key_mode is HKS_ALG_GCM or HKS_ALG_CBC or HKS_ALG_CCM;
 * key_param.key_pad is HKS_PADDING_NONE or or HKS_PADDING_PKCS7;
 * crypt_param:
 * crypt_param.nonce.size need set different values due to key_mode
 * crypt_param.aad.size need set different values due to key_mode
 * return 0 OK, other error
