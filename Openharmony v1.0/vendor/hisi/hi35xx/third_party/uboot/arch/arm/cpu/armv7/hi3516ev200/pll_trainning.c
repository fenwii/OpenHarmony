...
static void svb_voltage_regulator_init(struct voltage_regulator *regulator)
{
	...
	val = readl(regulator->reg_virt + SVB_SYS_VOLTAGE_OFFSET);
...
static int svb_voltage_adjust(struct voltage_regulator *regulator, int step)
{
	...
	writel(val, regulator->reg_virt + SVB_SYS_VOLTAGE_OFFSET);
	writel(NUM_5, regulator->reg_virt + SVB_SYS_VOLTAGE_UP_OFFSET);
	return 0;
}

static void svb_voltage_restore(struct voltage_regulator *regulator)
{
	unsigned int val = readl(regulator->reg_virt + SVB_SYS_VOLTAGE_OFFSET);
	val &= 0xffffff00;
	val |= regulator->curr;
	writel(val, regulator->reg_virt + SVB_SYS_VOLTAGE_OFFSET);
	writel(NUM_5, regulator->reg_virt + SVB_SYS_VOLTAGE_UP_OFFSET);
