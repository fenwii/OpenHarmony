...
static hi_s32 hal_hdmi_video_path_set(const struct hi_hdmi_hal_ *hal, const hdmi_video_config *video_cfg)
{
...
static hi_void hal_hdmi_ctrl_reset(const struct hi_hdmi_hal_ *hdmi_hal)
{
    ...
    hal_hdmi_ctrl_data_reset(hdmi_hal->hal_ctx.hdmi_id, HI_FALSE, 0);
...
#ifdef PHY_CRAFT_S28
static hi_s32 hal_hdmi_phy_hw_spec_get(const struct hi_hdmi_hal_ *hdmi_hal, hdmi_hw_spec *hw_spec)
{
    hi_s32 ret;
    ...
    if (ret == HI_SUCCESS) {
        memcpy(hw_spec, &phy_hw_param.hw_spec_cfg, sizeof(hdmi_hw_spec));
    }

    return ret;
}

static hi_s32 hal_hdmi_phy_hw_spec_set(const struct hi_hdmi_hal_ *hdmi_hal, hdmi_hal_hw_param param)
{
    hi_s32 ret;

    hdmi_if_fpga_return(HI_SUCCESS);
    hdmi_if_null_return(hdmi_hal, HI_FAILURE);
    hdmi_if_null_return(param.hw_spec, HI_FAILURE);
