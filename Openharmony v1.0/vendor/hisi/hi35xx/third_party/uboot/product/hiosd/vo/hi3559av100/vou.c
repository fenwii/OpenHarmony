...
static hi_u32 g_bg_color[4] = { 0x00ffff, 0xFF00, 0xFF00, 0xFF00 };
...
int start_gx(unsigned int layer, unsigned long addr, unsigned int strd, hi_vo_rect gx_rect)
{
    ...
    hal_graphic_set_gfx_stride(vo_layer, strd >> 4); /* 4 to set regitster */
...
hi_void video_layer_set_zme_cfg(unsigned int layer, const hi_rect *disp_rect)
{
...
hi_void video_layer_set_csc_by_intf(unsigned int layer, vo_intf_type intf_type)
{
    hi_vo_layer vo_layer;
...
int start_videolayer(unsigned int layer, unsigned long addr, unsigned int strd, hi_vo_rect layer_rect)
{
    ...
    hal_video_set_multi_area_enable(layer, 0, HI_TRUE);
    hal_layer_enable_layer(vo_layer, HI_TRUE);
...
int stop_videolayer(unsigned int layer)
{
    ...
    hal_video_set_multi_area_enable(layer, 0, HI_FALSE); /* area 0 */
    hal_layer_enable_layer(vo_layer, HI_FALSE);
    hal_layer_set_reg_up(hal_video_layer);
