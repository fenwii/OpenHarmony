 ...
 *    has_bias_value : Whether to have bias value. Defaults to false.
 *    filler_type    : If set to 'constant', there are two scenes:
 *                        filler_value will be used when filter set, bias not set and has_bias_value set true.
 *                        filler_value also will be used when filter not be set.
 *                     If not set to 'constant', filler_value will not be used and filter must be input.
 *    filler_value   : filler_value is used as the weight.
 * <Added in HiAI version>
 *    100.300.010.011
 */
CPT_REG_OP(Scale)
