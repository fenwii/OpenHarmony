...
class UserModelInterface():
    ...
    def get_input_placeholder (self):
        """Creates and returns the input placeholder of the model.

        Args:
        * None

        Returns:
        * outputs: a list, including image input placeholder and label input placeholder
        """
        pass #pylint: disable=W0107

    @abstractmethod
    def get_next_batch(self, is_train):
        """Reads the input of a batch. This function would be run in every iter.

        Args:
        * is_train: Boolean, True for training, False for evaluation

        Returns:
        * next_batch: A list of tensor, including the input image and label of the next batch
        """
        pass #pylint: disable=W0107

    @abstractmethod
    def forward_fn(self, inputs, is_train):
        """Reads the input in placeholder format, defines the forward inference process of a model,
           and returns the output of the forward inference.

        Args:
        * inputs: list returned by get_input_placeholder
        * is_train: Boolean, True for training, False for evaluation

        Returns:
        * outputs: A tensor, which is the output result of the forward inference.
        """
        pass #pylint: disable=W0107

    @abstractmethod
    def loss_op(self, inputs, outputs):
        """Defines and returns the loss function of the model.

        Args:
        * inputs: list returned by get_input_placeholder
        * outputs: forward inference result returned by forward_fn

        Returns:
        * loss: A tensor, that is, the user-defined loss
        """
        pass #pylint: disable=W0107

    @abstractmethod
    def metrics_op(self, inputs, outputs):
        """Defines the model evaluation method.
        ...
        * outputs: forward inference result returned by forward_fn
        ...
        Returns:
        * metrics: A tensor, which is a user-defined model evaluation method
        """
        pass #pylint: disable=W0107
    ...
    def config_lr_policy(self, global_step):
        ...
        * global_step: int
        ...
        Returns:
        * A tensor, including the learning rate
        """
        pass #pylint: disable=W0107
...
class UserModel(UserModelInterface):
    ...
    def __init__(self, dataset_dir, train_batch_size, test_batch_size, #pylint: disable=R0913
        train_batch_num, test_batch_num):
        """Constructor function."""
        super(UserModel, self).__init__(dataset_dir, train_batch_size,
    ...
    def forward_fn(self, inputs, is_train):
        ...
        * outputs: A tensor, which is the output result of the forward inference.
        ...
        outputs = tf.layers.conv2d(
            images, 32, [5, 5], padding='same', activation=tf.nn.relu, name='conv1')
        outputs = tf.layers.max_pooling2d(
            outputs, [2, 2], 2, padding='same', name='pool1')

        # conv2
        outputs = tf.layers.conv2d(
            outputs, 64, [5, 5], padding='same', activation=tf.nn.relu, name='conv2')
        outputs = tf.layers.max_pooling2d(
            outputs, [2, 2], 2, padding='same', name='pool2')

        # fc1
        outputs = tf.layers.flatten(outputs, name='flatten')
        outputs = tf.layers.dense(
            outputs, 1024, activation=tf.nn.relu, name='fc1')
    ...
    def metrics_op(self, inputs, outputs):
        ...
        * metrics: A tensor, which is a user-defined model evaluation method
        ...
        metrics = tf.reduce_mean(
            tf.cast(tf.nn.in_top_k(outputs, index, 1), tf.float32))
        return metrics
    ...
    def config_lr_policy(self, global_step):
        ...
        * A tensor, including the learning rate
        ...
        learning_rate = tf.train.piecewise_constant(
            global_step, boundaries, learning_rates)
